...
"""
-------------------------------------------------
   File Name：     stock_rank_xxtp
Description :   技术指标 -- 向下突破
   Author :       EveryFine
   Date：          2024/7/21
-------------------------------------------------
   Change Activity:
...
class StockRankXxtpBase(SQLModel):
...
class StockRankXxtp(StockRankXxtpBase, table=True):
    """stock_rank_xxtp表"""
    __tablename__ = "stock_rank_xxtp"
    id: int | None = Field(default=None, primary_key=True, description='id')


class StockRankXxtpCreate(StockRankXxtpBase):
    pass


class StockRankXxtpPublic(StockRankXxtpBase):
    id: int


class StockRankXxtpAllPublic(SQLModel):
    data: list[StockRankXxtpPublic]
    count: int