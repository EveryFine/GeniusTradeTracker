...
class StockHistoryBaoKHfqBase(SQLModel):
    ...
    adjust_flag: int | None = Field(default=None, description='复权状态(1：后复权， 2：前复权，3：不复权）此表全为1')
...
class StockHistoryBaoKHfq(StockHistoryBaoKHfqBase, table=True):
    """stock_history_bao_k_hfq表"""
    __tablename__ = "stock_history_bao_k_hfq"
    id: int | None = Field(default=None, primary_key=True, description='id')


class StockHistoryBaoKHfqCreate(StockHistoryBaoKHfqBase):
    pass


class StockHistoryBaoKHfqPublic(StockHistoryBaoKHfqBase):
    id: int


class StockHistoryBaoKHfqsPublic(SQLModel):
    data: list[StockHistoryBaoKHfqPublic]
    count: int