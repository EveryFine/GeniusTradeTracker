...
class StockHistoryBaoKQfqBase(SQLModel):
    ...
    adjust_flag: int | None = Field(default=None, description='复权状态(1：后复权， 2：前复权，3：不复权）此表全为2')
...
class StockHistoryBaoKQfq(StockHistoryBaoKQfqBase, table=True):
    """stock_history_bao_k_qfq表"""
    __tablename__ = "stock_history_bao_k_qfq"
    id: int | None = Field(default=None, primary_key=True, description='id')


class StockHistoryBaoKQfqCreate(StockHistoryBaoKQfqBase):
    pass


class StockHistoryBaoKQfqPublic(StockHistoryBaoKQfqBase):
    id: int


class StockHistoryBaoKQfqsPublic(SQLModel):
    data: list[StockHistoryBaoKQfqPublic]
    count: int
