...
def check_stock_fund_market_detail_date(session, check_date):
    last_trade_date = get_last_trade_date_by_date(session=session, final_date=check_date)
    statement = select(StockFundMarketDetail).where(
        StockFundMarketDetail.trade_date == last_trade_date)
    items = session.execute(statement).all()
