...
from app.crud.crud_stock_board_concept_em import get_stock_board_concept_symbol, get_unique_board_symbols_last_year
...
def create_stock_board_concept_cons_em(*, session: Session) -> int:
    ...
    for board_item in board_items:
        ...
        res = create_stock_board_concept_cons_em_item(session, board_symbol, board_name, trade_date, collect_time)
        create_count += res
...
def create_stock_board_concept_cons_em_by_board_name(*, session: Session, board_name) -> int:
    ...
    session.commit()
    log.info(
        f'creat stock board concept em(board_symbol:{board_symbol},board_name:{board_name}) finish, created count: {res}')
    return res


def create_stock_board_concept_cons_em_item(session, board_symbol, board_name, trade_date, collect_time):
    ...
    for index, stock_row in stock_board_concept_cons_em_df.iterrows():
        ...
        stock_index = stock_row['序号']
        latest_price = stock_row['最新价']
        change_rate = stock_row['涨跌幅']
        change_amount = stock_row['涨跌额']
        volume = stock_row['成交量']
        turnover = stock_row['成交额']
        range = stock_row['振幅']
        high = stock_row['最高']
        low = stock_row['最低']
        open = stock_row['今开']
        pre_close = stock_row['昨收']
        turnover_rate = stock_row['换手率']
        forward_pe_ratio = stock_row['市盈率-动态']
        pb_mrq = stock_row['市净率']
        items_saved = get_stock_board_concept_cons_em_items(session, board_symbol, stock_symbol, collect_time)
        if items_saved is None or len(items_saved) == 0:
            stock_board_concept_cons_em_create = StockBoardConceptConsEm(trade_date=trade_date,
...
def get_stock_board_concept_cons_em_items(session, board_symbol, stock_symbol, collect_time):
    statement = (select(StockBoardConceptConsEm).where(StockBoardConceptConsEm.board_symbol == board_symbol).
                 where(StockBoardConceptConsEm.stock_symbol == stock_symbol).where(
        StockBoardConceptConsEm.collect_time == collect_time))
