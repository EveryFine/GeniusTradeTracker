...
"""
-------------------------------------------------
   File Name：     crud_stock_history_qfq
   Description :
   Author :       EveryFine
   Date：          2024/7/14
-------------------------------------------------
...
def get_stock_histories(session, symbol):
    statement = select(StockHistoryQfq).where(StockHistoryQfq.symbol == symbol).order_by(StockHistoryQfq.date.desc())
...
def create_stock_hist(session, row):
    ...
    stock_hist_create = StockHistoryQfqCreate(symbol=symbol, date=date, open=open, close=close, high=high, low=low,
    ...
    db_stock_hist = StockHistoryQfq.model_validate(stock_hist_create)
    session.add(db_stock_hist)
    # session.commit()
    # session.refresh(db_stock_hist)
    res = StockHistoryQfq.model_validate(db_stock_hist)
    return res


def check_stock_history_qfq_date(session, check_date):
    last_trade_date = get_last_trade_date_by_date(session=session, final_date=check_date)
    statement = select(StockHistoryQfq).where(
        StockHistoryQfq.date == last_trade_date)
