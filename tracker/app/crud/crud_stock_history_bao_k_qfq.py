...
"""
-------------------------------------------------
   File Name：     crud_stock_history_bao_k_qfq
   Description :
   Author :       EveryFine
   Date：          2025/5/2
-------------------------------------------------
...
from app.models.stock_history_bao_k_qfq import StockHistoryBaoKQfqCreate, StockHistoryBaoKQfq


def create_stock_history_bao_k_qfq(*, session: Session) -> int:
...
def create_part_stock_bao_k_qfq(*, session: Session, stock_offset: int = 0,
...
def get_stock_histories(session, symbol):
    statement = select(StockHistoryBaoKQfq).where(StockHistoryBaoKQfq.symbol == symbol).order_by(
        StockHistoryBaoKQfq.date.desc())
    stock_hists = session.execute(statement).all()
    return stock_hists


def create_stock_hist_bao_k_qfq(session, row, symbol, name):
    code = row['code']
    date = row['date']
    ...
    stock_hist_bao_k_qfq_create = StockHistoryBaoKQfqCreate(code=code,
                                                            symbol=symbol,
                                                            name=name,
                                                            date=date,
                                                            open=open,
    ...
    db_stock_hist = StockHistoryBaoKQfq.model_validate(stock_hist_bao_k_qfq_create)
    session.add(db_stock_hist)
    # session.commit()
    # session.refresh(db_stock_hist)
    res = StockHistoryBaoKQfq.model_validate(db_stock_hist)
    return res
...
def check_stock_history_date(session, check_date):
    ...
    statement = select(StockHistoryBaoKQfq).where(
        StockHistoryBaoKQfq.date == last_trade_date)
