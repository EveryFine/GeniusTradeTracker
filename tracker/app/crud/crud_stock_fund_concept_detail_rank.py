...
"""
-------------------------------------------------
   File Name：     crud_stock_fund_concept_detail_rank
...
def create_stock_fund_concept_detail_rank(*, session: Session) -> int:
    range_types = (
        "5日", "10日")
    total_count = create_stock_fund_concept_detail_rank_by_types(session, range_types)
    return total_count


def create_stock_fund_concept_detail_rank_by_types(session, range_types):
    total_count = 0
    for range_type in range_types:
        res = create_stock_fund_concept_detail_rank_by_type(session, range_type)
        total_count += res
        session.commit()
        log.info(
            f'creat stock fund concept detail rank(资金流--概念--详细--排行) by range type finish, type:{range_type}, created count:{res}')
    session.commit()
    log.info(f'creat stock fund concept detail rank(资金流--概念--详细--排行) finish, total count: {total_count}')
    return total_count


def create_stock_fund_concept_detail_rank_by_type(session, range_type) -> int:
    type_count = 0
    trade_date = get_last_trade_date(session=session, final_datetime=datetime.datetime.now())
    stock_fund_concept_detail_rank_ths_df = ak.stock_sector_fund_flow_rank(indicator=range_type,
                                                                           sector_type="概念资金流")
    for index, row in stock_fund_concept_detail_rank_ths_df.iterrows():
        res = create_stock_fund_concept_detail_rank_item(session, trade_date, row, range_type)
        type_count += res
...
def create_stock_fund_concept_detail_rank_item(session, trade_date, row, range_type):
    name = row['名称']
    main_in_rank = row['序号']
    if range_type == '5日':
...
def get_stock_fund_concept_detail_rank_items(session, name, trade_date, range_type):
    statement = (select(StockFundConceptDetailRank).where(StockFundConceptDetailRank.name == name).where(
        StockFundConceptDetailRank.trade_date == trade_date).where(
