...
def create_histories_by_list(session, stock_infos):
    ...
    for stock_info in stock_infos:
        ...
        try:
            ...
            error_msg = f"{datetime.now()} history bao k hfq processing data for {code} from {start_date} to {end_date} error: {str(e)}\n{traceback.format_exc()}"
            log.error(error_msg)
            bs.logout()
...
def get_latest_stock_history(session, symbol):
    # 只获取最新的一条记录
    statement = (
        select(StockHistoryBaoKHfq)
        .where(StockHistoryBaoKHfq.symbol == symbol)
        .order_by(StockHistoryBaoKHfq.date.desc())
        .limit(1)  # 限制只返回一条记录
    )
