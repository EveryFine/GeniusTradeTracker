...
"""
-------------------------------------------------
   File Name：     crud_stock_board_industry_em
   Description :
   Author :       EveryFine
   Date：          2025/9/7
-------------------------------------------------
...
def create_stock_board_industry_em(*, session: Session) -> int:
    create_count = 0
    ...
    for index, row in stock_board_industry_em_df.iterrows():
        res = create_stock_board_industry_em_item(session, trade_date, collect_time, row)
        create_count += res
...
def create_stock_board_industry_em_item(session, trade_date, collect_time, row):
    rank = row['排名']
    name = row['板块名称']
    symbol = row['板块代码']
    ...
    items_saved = get_stock_board_industry_em_items(session, symbol, trade_date)
    if items_saved is None or len(items_saved) == 0:
        stock_board_industry_em_create = StockBoardIndustryEm(trade_date=trade_date,
        ...
        db_stock_board_industry_em = StockBoardIndustryEm.model_validate(stock_board_industry_em_create)
        session.add(db_stock_board_industry_em)
        return 1
    else:
        return 0
