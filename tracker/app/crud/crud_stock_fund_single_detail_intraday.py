...
"""
-------------------------------------------------
   File Name：     crud_stock_fund_single_detail_intraday
...
from app.models.stock_fund_single_detail_intraday import StockFundSingleDetailIntraday


def create_stock_fund_single_detail_intraday(*, session: Session) -> int:
    total_count = 0
    ...
    for index, row in stock_fund_single_detail_intraday_ths_df.iterrows():
        res = create_stock_fund_single_detail_intraday_item(session, trade_date, row)
        total_count += res
        if total_count > 0 and total_count % 100 == 0:
            session.commit()
    session.commit()
    log.info(f'creat stock fund single detail intraday(个股资金流详细--即时) finish, created count: {total_count}')
    return total_count


def create_stock_fund_single_detail_intraday_item(session, trade_date, row):
    symbol = row['代码']
    name = row['名称']
    ...
    items_saved = get_stock_fund_single_detail_intraday_items(session, symbol, trade_date)
    if items_saved is None or len(items_saved) == 0:
        stock_fund_single_detail_intraday_create = StockFundSingleDetailIntraday(trade_date=trade_date,
                                                                                 symbol=symbol,
                                                                                 name=name,
        ...
        db_stock_fund_single_detail_intraday = StockFundSingleDetailIntraday.model_validate(
...
def get_stock_fund_single_detail_intraday_items(session, symbol, trade_date):
    statement = (select(StockFundSingleDetailIntraday).where(StockFundSingleDetailIntraday.symbol == symbol).
                 where(StockFundSingleDetailIntraday.trade_date == trade_date))
...
def check_stock_fund_single_detail_intraday_date(session, check_date):
    last_trade_date = get_last_trade_date_by_date(session=session, final_date=check_date)
    statement = select(StockFundSingleDetailIntraday).where(
        StockFundSingleDetailIntraday.trade_date == last_trade_date)
    items = session.execute(statement).all()
