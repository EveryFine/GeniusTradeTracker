...
"""
-------------------------------------------------
   File Name：     crud_stock_fund_industry_detail_intraday
...
from app.models.stock_fund_industry_detail_intraday import StockFundIndustryDetailIntraday


def create_stock_fund_industry_detail_intraday(*, session: Session) -> int:
    total_count = 0
    trade_date = get_last_trade_date(session=session, final_datetime=datetime.datetime.now())
    stock_fund_industry_detail_intraday_ths_df = ak.stock_sector_fund_flow_rank(indicator="今日",
                                                                                sector_type="行业资金流")
    for index, row in stock_fund_industry_detail_intraday_ths_df.iterrows():
        res = create_stock_fund_industry_detail_intraday_item(session, trade_date, row)
        total_count += res
        if total_count > 0 and total_count % 100 == 0:
            session.commit()
    session.commit()
    log.info(
        f'creat stock fund industry detail intraday(资金流--行业--详细--即时) finish, created count: {total_count}')
    return total_count


def create_stock_fund_industry_detail_intraday_item(session, trade_date, row):
    name = row['名称']
    ...
    items_saved = get_stock_fund_industry_detail_intraday_items(session, name, trade_date)
    if items_saved is None or len(items_saved) == 0:
        stock_fund_industry_detail_intraday_create = StockFundIndustryDetailIntraday(trade_date=trade_date,
                                                                                     name=name,
        ...
        db_stock_fund_industry_detail_intraday = StockFundIndustryDetailIntraday.model_validate(
...
def get_stock_fund_industry_detail_intraday_items(session, name, trade_date):
    statement = (select(StockFundIndustryDetailIntraday).where(StockFundIndustryDetailIntraday.name == name).
                 where(StockFundIndustryDetailIntraday.trade_date == trade_date))
    items = session.execute(statement).all()
    return items


def check_stock_fund_industry_detail_intraday_date(session, check_date):
    last_trade_date = get_last_trade_date_by_date(session=session, final_date=check_date)
    statement = select(StockFundIndustryDetailIntraday).where(
        StockFundIndustryDetailIntraday.trade_date == last_trade_date)
    items = session.execute(statement).all()
